...
def execute_sql(table_name, ncolumns, colsize, duration, nrows):
    ...
    while time.time() - start < duration:
        try:
            ...
            cnt = 0
            while time.time() - start < duration:
                # Adjust to MySql-syntax
                colid = random.randint(0, ncolumns - 1)
                rowid = random.randint(1, nrows - 1)
                value = "".join(
                    str(round(random.randint(100, 999))) for _ in range(rpt)
                )  # HINT: the equivalent version to the original code should be randint(0, 999)
                sql = f"update {table_name} set name{colid}='{value}' where id={rowid};"
                # "name{}=repeat(round(random()*999)::text,{}) where id={};".format(
                #     colid, rpt, rowid
                # )
                cursor.execute(sql)
                cnt += 1
    ...
    while time.time() - start < duration:
        try:
            ...
            while time.time() - start < duration:
                ...
                xx = ""
                rpt = colsize // 3
                xx += "".join(str(random.randint(100, 999)) for _ in range(rpt))
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--nrows", type=int, default=10, help="number of rows per insertion"
    )
