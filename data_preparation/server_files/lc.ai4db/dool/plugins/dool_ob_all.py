...
class dool_plugin(dool):
    ...
    def extract(self):
        try:
            c = self.db.cursor()
            c.execute("""select name, value from gv$sysstat where con_id = 1;""")
            rows = c.fetchall()  # 获取所有行

            # 遍历每一行，将其存储在 self.val 中
            for row in rows:
                name, value = row  # 分解行为 name 和 value
                if name in self.vars:  # 如果 name 在 self.vars 中定义
                    self.val[name] = value  # 更新 self.val
            # 可选：对于进一步处理或调试，您可以打印或返回这些结果
        #            print(self.val)
        # return self.val
