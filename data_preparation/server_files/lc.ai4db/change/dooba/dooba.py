...
def save_to_csv(data):
    """
    Saves the provided data entry to a CSV file. Writes the header if the file doesn't exist.

    :param entry: A single data entry, a list containing time, tnt, ip, port, name, value.
    :param filename: The name of the file to save the data to. Default is 'data.csv'.
    """
    # 检查文件是否存在，以决定是否需要写入列标题
    filename="data.csv"
    fieldnames = data[0].keys()
    file_exists = os.path.exists(filename)

    with open(filename, 'a') as file:  # 使用 'a' 模式以追加数据
...
class OceanBase(object):
    ...
    def __sub_stat(self, now, prev): # 差分处理数据
        r = {}
        for k in now.keys():
    ...
    def redraw(self):
        ...
        for i in range(0, len(latest)):
            ...
            for c in enabled_columns:
                ...
                row_data[c] = str(c.value(latest[i]))
            # name_tmp = self.__name
            li.append(" " + " ".join(item))
